...
set out [file join $fpga build variable_fused_point_source_sim]
file mkdir $out
...
set files [list \
 ...
 [file join $fpga sim tb_ch4_variable_fused_point_source.v]]
exec xvlog {*}$files
exec xelab --timescale 1ns/1ps tb_ch4_variable_fused_point_source -s variable_fused_point_source_sim
set result [exec xsim variable_fused_point_source_sim -runall]
puts $result
if {[string first "VARIABLE_FUSED_POINT_SOURCE_PASS" $result] < 0} {error "variable fused point source test failed"}
