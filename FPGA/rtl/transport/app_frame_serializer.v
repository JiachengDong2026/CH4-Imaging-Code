...
module app_frame_serializer #(parameter integer PAYLOAD_BYTES=48)(input wire clk,rst,start,input wire[7:0]dst,src,msg_type,flags,
 ...
 output wire byte_valid,output wire[7:0]byte_data,output reg busy,output wire done);
 ...
 assign byte_data=current;
 assign done=busy&&byte_ready&&(index==FRAME_BYTES-1);
 always @(posedge clk)if(rst)begin index<=0;crc<=16'hffff;latched<=0;busy<=0;end else begin
  if(start&&!busy)begin index<=0;crc<=16'hffff;latched<=payload;busy<=1;end else if(busy&&byte_ready)begin
   if(index>=2&&index<11+PAYLOAD_BYTES)crc<=crcbyte(crc,current);if(index==FRAME_BYTES-1)busy<=0;else index<=index+1'b1;end end
endmodule
